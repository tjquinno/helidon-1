...
public class GreetService implements Service {
    ...
    static final long SLOW_ENDPOINT_DELAY_SECONDS = 2;
    ...
    private void respondSlowly(ServerRequest request, ServerResponse response) {
        if (slowRequestInProgress != null) {
            slowRequestInProgress.countDown();
        }
        try {
            TimeUnit.SECONDS.sleep(SLOW_ENDPOINT_DELAY_SECONDS);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        sendResponse(response, "Slowpoke");
    }
